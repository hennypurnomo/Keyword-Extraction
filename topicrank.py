'''
This system is made by Adrien Bougouin, Florian Boudin and Béatrice Daille. 2013. 
in Topicrank: Graph-Based Topic Ranking for keyphrase Extraction. 
In Proceedings of the 6th International Joint Conference on Natural Language Processing (IJCNLP), 
Nagoya, Japan, October.

This code is implemented into python by smirnov-am
from https://github.com/smirnov-am/pytopicrank

Some changes are made in this code, such as:
1. Get top n was modified to return all the keywords from its cluster.
	Because in the paper, it states if the number of cluster (all keywords from a cluster) same like n top candidate,
	it will get a highest fmeasure.
2. Change the stemmer from snowball to porter
3. Because there is some error when extracting label on news dataset (about detecting language),
	some code are commented and changed.
'''
...
class TopicRank:
	...
	def get_top_n(self, n=1, cluster_strategy='average', max_d=1.25, extract_strategy='first'):
        ...
        :param strategy: How to select keyphrase from topic:
                         -first - use the one which appears first (default one)
		...
		for rank, topic in self.topics[:n]:
			if topic:
                #change into all keywords
				first_kp = topic 
				# the default setting is sorted(topic, key=lambda x: self.phrases[x][0])[0]
                #unstem_kp_sort = sorted([self.unstem_map[i] for i in first_kp.split(' ')])
                #unstem_kp = ' '.join([i[1] for i in unstem_kp_sort])
				result.append(first_kp)
		return result
